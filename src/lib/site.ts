...
export const site = {
  ...
  aboutLong:
    "**빠른 환경 변화 속 문제 정의와 현장 판단**\n\n정보를 빠르게 분석해 결과를 얻는 일은 기술이 사람을 앞서는 시대가 되었지만, **데이터로 설명되지 않는 현장의 맥락을 읽어내는 것**은 여전히 사람의 역할로 남아 있습니다.\n\n저는 전자금융거래법 망 환경에서 대규모 서비스를 운영하며, **문제를 정의하고 리스크를 감지하는 판단력**을 길러왔습니다. 규제와 혁신 사이에서 현실적인 균형점을 찾아내며 프로젝트를 이끌어 왔습니다.\n\n특히, 금융 IT 환경은 지식이 말이나 글로 명확히 표현되기 어려워, 담당자 개인의 경험이나 통찰에 의존해 운영되는 경우가 많습니다.\n\n저는 이렇게 암묵적으로 사용되던 기준들을 표준 절차와 가이드로 체계화하여, 조직 전체가 일관된 판단을 내릴 수 있는 구조를 만드는 데 강점이 있습니다.\n\n\n\n**대규모 핀테크 플랫폼 운영 체계의 기틀 마련**\n\n\n핀테크 플랫폼 운영(DevOps) 팀의 초기 멤버로서, 전자금융거래법 망 환경에서 내부통제 요건을 준수하는 안정적인 서비스 운영 체계를 구축해 왔습니다.\n\n** | 배치 통제 체계 구현**\n부서별로 독립 운영되던 Jenkins 환경을 중앙 통합하고, 권한 관리부터 이슈 대응까지의 절차를 새롭게 정리했습니다.\n정보보호 및 개발팀과의 협업해, 감사 대응과 운영 효율이 실제 현장에서 함께 작동할 수 있는 내부통제 프로세스를 수립했습니다.\n\n**| 쿠버네티스 기반 배포 지원**\n전금법 망 환경의 통제 흐름과 승인 절차를 이해한 상태에서, 사내 표준 배포 도구를 활용해 배포 업무를 수행했습니다.\n\n이 외에도 게이트웨이, 아티팩트, 사내 AI 도구 등 다양한 사내 플랫폼을 운영·활용하며, 금융망 환경에서 각 시스템이 어떻게 적용되고 연계되어 동작하는지에 대한 이해를 쌓아왔습니다.\n\n**제한된 환경 속에서 인프라 효율을 극대화**\n\n모니터링 플랫폼 Sentry 운영 중, 이슈 생성 지연이 최대 9시간까지 발생했고 **데이터 분석**을 통해 병목 지점을 파악했습니다.\n\n분석 결과를 바탕으로 데이터 분배 로직을 최적화하고 배포 구조를 개선한 결과,\n**이슈 생성 지연을 9시간에서 1분 이내로 단축**하고 미처리 데이터를 **94%(2,800만 건 → 162만 건) 감소**켰습니다.\n\n이를 통해 기존 초당 3,000건 수준이었던 **데이터 처리 용량을 초당 12,000건으로 4배 이상 확장**하며 시스템 가용성을 획기적으로 높였습니다.\n\n이처럼 운영 업무 속에서 실질적인 성과를 수치로 증명할 수 있는 개선 경험을 다수 보유하고 있습니다.\n\n**변화에 따른 선제적 판단과 실행력**\n\n또한, 클라우드 이전과 같은 대규모 환경 변화에서는 가장 먼저 실무 적용 방안을 정립하고, 팀원들의 현실을 고려한 협업 구조를 설계했습니다.\n\n글로 설명하기 어려운 현장의 노하우들을 누구나 보고 따라 할 수 있는 가이드와 표준 절차로 바꿔\n구성원들이 동일한 기준을 따르도록 이끌어 변화 상황에서도 혼란을 최소화하며 유관 부서보다 한발 앞서 대응할 수 있었습니다.\n\n\n\n저는 이렇게 개발부터 운영까지 전 과정을 아우르며, 안정적인 서비스 환경을 설계해 왔습니다.\n다양한 개발 언어와 DB·메시징 시스템, CI/CD, 모니터링을 프로젝트 성격에 맞게 조합하며 핀테크 플랫폼 전반의 개선을 직접 시도하고 검증해 왔습니다. 이를 통해 IT 프로세스 전 과정에서 특정 기술에 국한되지 않은 실무 중심의 해법을 제시할 수 있습니다.\n\n이러한 역량들을 바탕으로 IT Planning Manager로서, 규제 환경 안에서도 혁신하는 최적 경로를 만들어가겠습니다.",
  aboutKeywords: [
    "핀테크 플랫폼 기획·개발·운영 경험",
    "IT 내부통제/변경관리",
    "운영 매뉴얼·실습 커리큘럼 제작",
